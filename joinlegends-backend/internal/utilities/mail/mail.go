...
func SendConfirmationChangePassword(name, email, loginUrl string) error {
	message := gomail.NewMessage()
	...
	<head>
		<meta charset="UTF-8" />
		<title>Conta Recuperada com Sucesso</title>
		<style>
			body {
				font-family: Arial, sans-serif;
				background-color: #f7f7f7;
	...
	<body>
		<div class="container">
			<h2>Olá, %s 👋</h2>
			<p>Sua senha da <strong>JWT-Session</strong> foi redefinida com sucesso.</p>
			<p>Agora você já pode acessar sua conta normalmente clicando no botão abaixo:</p>
			<a href="%s" class="button">Acessar Conta</a>
			<p>Se você não realizou essa ação, recomendamos alterar sua senha imediatamente.</p>
			<div class="footer">
				<p>© 2025 JWT-Session. Todos os direitos reservados.</p>
			</div>
		</div>
